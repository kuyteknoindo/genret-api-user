...
const MainApp: React.FC = () => {
    ...
    const [manualPreviewData, setManualPreviewData] = useState<{
        textPrompt: string;
        imageUrl: string | null;
        isLoading: boolean;
        error: string | null;
    ...
    const handleGenerateCasualPreview = async () => {
        ...
        try {
            ...
            const imageUrl = await performApiCall(apiKey => generateImage(apiKey, imageGenPrompt), onUpdate);
    ...
    const handleGenerateAdatPreview = async () => {
        ...
        if (!region) {
            setAdatPreviewData(prev => ({ ...(prev!), error: "Harap pilih daerah dari daftar." }));
            return;
        }
    
        const inspiration = D.adatInspirations.find(item => item.region === region);
        if (!inspiration) {
            setAdatPreviewData(prev => ({ ...(prev!), error: "Data inspirasi untuk daerah yang dipilih tidak ditemukan." }));
            return;
        }
    
        previewCancellationRef.current = false;
        
        const textPrompt = `A young Indonesian couple in authentic traditional wedding attire from ${inspiration.region}. The bride wears: ${inspiration.bride}. The groom wears: ${inspiration.groom}. They are surrounded by elements of their culture, like the ${inspiration.background}, and their clothing is made from traditional ${inspiration.fabrics}.`;
        
        setAdatPreviewData(prev => ({ ...(prev!), imageUrl: null, textPrompt: textPrompt, isLoading: true, error: null, statusText: `Membuat preview gambar untuk ${region}...` }));
        ...
        try {
            ...
            const imageGenPrompt = `Photorealistic 4k cinematic preview, 3:4 aspect ratio. Description: "${textPrompt}". **CRITICAL: The couple must be ethnically Indonesian, aged around 19-23 years old, with features authentic to the ${region} region.** Culturally accurate attire. No cartoons.`;
            const imageUrl = await performApiCall(apiKey => generateImage(apiKey, imageGenPrompt), onUpdate);
    ...
    const handleGenerateManualPreview = async () => {
        if (!prompt) {
            setManualPreviewData(prev => ({ ...(prev!), error: "Prompt tidak boleh kosong."}));
            return;
        }

        previewCancellationRef.current = false;
        // Keep the original prompt text in the state for display
        setManualPreviewData({ textPrompt: prompt, imageUrl: null, isLoading: true, error: null, statusText: 'Membuat preview gambar...' });

        try {
            const onUpdate = (status: string) => {
                if (previewCancellationRef.current) return;
                setManualPreviewData(p => ({ ...(p!), statusText: status }));
            };

            const imageGenPrompt = `Photorealistic 4k cinematic preview, 3:4 aspect ratio. A young Indonesian couple, aged around 19-23 years old, their appearance and clothing are described as: "${prompt}". **Must be ethnically Indonesian.** Only one man and one woman. No cartoons.`;
            const imageUrl = await performApiCall(apiKey => generateImage(apiKey, imageGenPrompt), onUpdate);

            if (previewCancellationRef.current) return;
            setManualPreviewData(p => ({ ...(p!), imageUrl, isLoading: false, error: null, statusText: '' }));
            ...
            if (e.message.startsWith("ALL_KEYS_FAILED:")) {
                 setIsAllKeysFailedModalOpen(true);
                 setManualPreviewData(prev => ({ ...(prev!), isLoading: false, error: `Gagal membuat preview: Kunci API tidak valid atau habis.`, statusText: '' }));
            } else {
                const errorMessage = e.message;
                setManualPreviewData(prev => ({ ...(prev!), isLoading: false, error: `Gagal membuat preview: ${errorMessage}`, statusText: '' }));
            }
        } finally {
            if (previewCancellationRef.current) return;
            setActiveApiKeyMasked(null);
    ...
    const handleUseInspiration = (text: string, imageUrl: string) => {
        setPrompt(''); // Clear the prompt/notes field. The image is the primary reference.
    ...
    const runGeneration = async (isContinuation = false, overrideCount?: number) => {
        ...
        if (activeTab === 'prompt' && !prompt && !consistentCoupleDescription) {
            setModals(prev => ({ ...prev, error: 'Harap isi deskripsi pasangan di tab "Teks Prompt".' }));
            return;
        }

        const effectiveLocationTheme = useCustomTheme ? customLocationTheme.trim() : locationTheme;
        if (useCustomTheme && !effectiveLocationTheme) {
        ...
        try {
            ...
            setStatusText(`Membuat skenario kreatif untuk ${effectiveLocationTheme}...`);
            try {
                scenarios = await performApiCall(apiKey => generateLocationBasedScenarios(apiKey, effectiveLocationTheme, countForThisRun), setStatusText);
            } catch (error) {
            ...
            for (let i = startIndex; i < targetCount; i++) {
                ...
                const useVisualReference = 
                    ...
                    (activeTab === 'prompt' && sessionReferenceImageRef.current);
...
- New Scene (${effectiveLocationTheme}): ${scenario.scene}
- Emotion: ${scenario.emotion}
- Style: ${photoStyle}
- **Pose & Realism:** Ensure poses are natural, physically plausible, and reflect genuine emotional connection. Avoid awkward or impossible body positions.
${prompt && isReferenceTabActive ? `- User Notes: ${prompt}\n` : ''}- Negative Prompts: ${negativePrompt || 'None'}`;
                    imageUrl = await performApiCall(apiKey => generateImage(apiKey, finalPrompt, currentReference.base64, currentReference.mimeType), setStatusText);
                } else {
...
- **Location:** ${effectiveLocationTheme}
...
- **Negative Prompts:** ${negativePrompt || 'None'}`;
                    ...
                    imageUrl = await performApiCall(apiKey => generateImage(apiKey, finalPrompt), setStatusText);
    
                    if (activeTab === 'prompt' && i === startIndex) {
                        const [header, base64] = imageUrl.split(',');
                        const mimeType = header.match(/:(.*?);/)?.[1] || 'image/jpeg';
            ...
            <div className="space-y-6 flex-grow">
                <div>
                    <div className="flex justify-between items-center mb-2">
                        <label htmlFor="location-theme" className="block text-sm font-medium text-gray-300">2. Pilih Tema Sesi Foto</label>
                        <button 
                        ...
                        >
                            {useCustomTheme ? 'Pilih dari Tema' : 'Gunakan Lokasi Kustom'}
                        </button>
                    </div>
                    {useCustomTheme ? (
                        <input 
                            ...
                            className="w-full bg-gray-900 border border-gray-700 rounded-lg p-3 text-sm text-gray-200 focus:ring-lime-500 focus:border-lime-500 transition-colors"
                            placeholder="Contoh: Piknik di bawah Menara Eiffel"
                        />
                    ) : (
                ...
                <div>
                    <label className="block text-sm font-medium text-gray-300 mb-2">3. Hindari Elemen (Negative Prompt)</label>
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <label htmlFor="image-count" className="block text-sm font-medium text-gray-300 mb-2">4. Jumlah Foto</label>
    ...
    return (
        <div className="h-screen bg-black text-gray-200 p-4 lg:p-6 flex flex-col lg:flex-row gap-6 relative">
                           ...
                           <div>
                                ...
                                <div className="flex justify-between items-center mt-2">
                                    <div className="flex gap-2">
                                        <button onClick={() => setPreviewData({ textPrompt: '', imageUrl: null, isLoading: false, error: null, statusText: '' })} disabled={hasApiKeyIssue} className="text-xs font-semibold bg-gray-800 border border-gray-700 px-3 py-1.5 rounded-md hover:bg-gray-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">Inspirasi Casual</button>
                                        <button onClick={() => setAdatPreviewData({ region: '', textPrompt: '', imageUrl: null, isLoading: false, error: null, statusText: '' })} disabled={hasApiKeyIssue} className="text-xs font-semibold bg-gray-800 border border-gray-700 px-3 py-1.5 rounded-md hover:bg-gray-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">Inspirasi Adat</button>
                                    </div>
                                    <div className="flex gap-2">
                                        <button onClick={() => setManualPreviewData({ textPrompt: prompt, imageUrl: null, isLoading: false, error: null, statusText: '' })} disabled={!prompt.trim() || hasApiKeyIssue} className="text-xs font-semibold bg-blue-600 text-white px-3 py-1.5 rounded-md hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                                            Preview
                                        </button>
                                        <button onClick={handleEnhancePrompt} disabled={isEnhancing || !prompt.trim() || hasApiKeyIssue} className="text-xs font-semibold bg-gray-800 border border-gray-700 px-3 py-1.5 rounded-md hover:bg-gray-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">{isEnhancing ? 'Meningkatkan...' : 'Enhanced'}</button>
                                    </div>
                                </div>
                           </div>
                           {commonFormElements}
                        ...
                        <div className="flex-grow overflow-y-auto custom-scrollbar -mr-4 pr-4">
                             <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                                {generatedImages.map(image => (
                                    <div key={image.id} className="relative group aspect-[4/5] cursor-pointer" onClick={() => setModals(prev => ({...prev, lightbox: image.url}))}>
                                ...
                                {isLoading && (
                                    <div className="aspect-[4/5] bg-gray-900 rounded-xl flex flex-col items-center justify-center text-center p-4">
            ...
            <CommonModals
                ...
                setManualPreviewData={setManualPreviewData}
                handleGenerateManualPreview={handleGenerateManualPreview}
